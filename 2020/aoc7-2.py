...
def run():
    input = """bright olive bags contain 5 dotted white bags, 2 wavy lavender bags.
...
dotted bronze bags contain 2 muted tomato bags.
    ...
    numContainers = getNumContainers(bags, key)
    print(numContainers)
...
def parseBags(input):
    bags = {}
    while input != "":
        line, input = parseNext(input, "\n")
        includingBag, line = parseNext(line, " bags contain ")
        line = line.replace("bags", "bag").replace(", ", "").replace(".", "").replace("no other ", "")
        while line != "":
            bagsInfo, line = parseNext(line, " bag")
            number, includedBag = parseNext(bagsInfo, " ")
            if includedBag != "":
                bags = addIncludedBag(bags, includingBag, includedBag, number)
    return bags


def initiateBag(bags, bagName):
...
def addIncludedBag(bags, includingBag, includedBag, number):
    bags = initiateBag(bags, includingBag)
    bags = initiateBag(bags, includedBag)
    if includedBag not in bags[includingBag]["directlyIncludes"]:
        bags[includingBag]["directlyIncludes"][includedBag] = int(number)
    if includingBag not in bags[includedBag]["directlyIncludedIn"]:
        bags[includedBag]["directlyIncludedIn"][includedBag] = int(number)
    return bags


def parseNext(input, delimiter):
    endIndex = input.find(delimiter)
    if endIndex != -1:
        group = input[0:endIndex]
        newInput = input[endIndex + len(delimiter):]
    else:
        group = input
        newInput = ""    
    return group, newInput


def getNumContainers(bags, key):
    directlyContains = bags[key]["directlyIncludes"]
    if len(directlyContains.keys()) == 0:
        return 0
    numBags = 0
    for bag, number in directlyContains.items():
        numBags = numBags + number + (number * getNumContainers(bags, bag))
    return numBags
