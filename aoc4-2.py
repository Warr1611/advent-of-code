...
def run():
    ...
    numValidPassports = 0
    while inputString != "":
        passport, inputString = parseNextPassport(inputString)
        passportValid = False
        if ("byr" in passport and byrIsValid(passport)) and \
            ("iyr" in passport and iyrIsValid(passport)) and \
            ("eyr" in passport and eyrIsValid(passport)) and \
            ("hgt" in passport and hgtIsValid(passport)) and \
            ("hcl" in passport and hclIsValid(passport)) and \
            ("ecl" in passport and eclIsValid(passport)) and \
            ("pid" in passport and pidIsValid(passport)):
            passportValid = True
...
def getValue(key, passport):
    index = passport.index(key)
    valueStart = passport.index(":", index) + 1
    nextSpace = passport.find(" ", valueStart)
    nextNewLine = passport.find("\n", valueStart)
    if nextSpace != -1 and nextNewLine != -1:
        if nextSpace < nextNewLine:
            valueEnd = nextSpace
        else:
            valueEnd = nextNewLine
    elif nextSpace == -1 and nextNewLine == -1:
        valueEnd = -1
    elif nextSpace == -1:
        valueEnd = nextNewLine
    else:
...
def byrIsValid(passport):
    value = int(getValue("byr", passport))
    if value >= 1920 and value <= 2002:
        return True
    return False


def iyrIsValid(passport):
    value = int(getValue("iyr", passport))
    if value >= 2010 and value <= 2020:
        return True
    return False


def eyrIsValid(passport):
    value = int(getValue("eyr", passport))
    if value >= 2020 and value <= 2030:
        return True
    return False


def hgtIsValid(passport):
    value = getValue("hgt", passport)
    cmIndex = value.find("cm")
    inIndex = value.find("in")
    if cmIndex != -1:
        cms = int(value[:cmIndex])
        if cms >= 150 and cms <= 193:
            return True
    if inIndex != -1:
        ins = int(value[:inIndex])
        if ins >= 59 and ins <= 76:
            return True
    return False


def hclIsValid(passport):
    value = getValue("hcl", passport)
...
def eclIsValid(passport):
    value = getValue("ecl", passport)
    validValues = [
        "amb",
        "blu",
        "brn",
...
def pidIsValid(passport):
    value = getValue("pid", passport)
    try:
        valInt = int(value)
        if len(value) == 9:
            return True
...
def parseNextPassport(input):
    endOfPassport = input.find("\n\n")
    if endOfPassport != -1:
        passportString = input[0:endOfPassport]
        input = input[endOfPassport + 2:]
    else:
        passportString = input
        input = ""    
    return passportString, input
