...
dotted bronze bags contain 2 muted tomato bags.
    ...
    containers = getContainers(bags, key)
    numContainers = len(set(containers))
    print(numContainers)
...
def parseBags(input):
    ...
    while input != "":
        ...
        line = line.replace("bags", "bag").replace(", ", "").replace(".", "")
        while line != "":
            bagsInfo, line = parseNext(line, " bag")
            number, includedBag = parseNext(bagsInfo, " ")
            bags = addIncludedBag(bags, includingBag, includedBag)
    return bags
...
def addIncludedBag(bags, includingBag, includedBag):
    ...
    if includedBag not in bags[includingBag]["directlyIncludes"]:
        bags[includingBag]["directlyIncludes"].append(includedBag)
    if includingBag not in bags[includedBag]["directlyIncludedIn"]:
        bags[includedBag]["directlyIncludedIn"].append(includingBag)
    return bags
...
def getContainers(bags, key):
    directlyContainedIn = bags[key]["directlyIncludedIn"]
    containers = directlyContainedIn
    for bag in directlyContainedIn:
        containers = containers + getContainers(bags, bag)
    return containers
