...
def run():
    ...
    for ruleString, password in input:
        rule = parseRule(ruleString)
        valid = checkPasswordValidity(rule, password)
        if valid:
...
def parseRule(ruleString):
    ...
    minimum, maximum = parts[0].split("-")
    character = parts[1]
    return (int(minimum), int(maximum), character)


def checkPasswordValidity(rule, password):
    if (password[rule[0] - 1] == rule[2] and not password[rule[1] - 1] == rule[2]) or \
    (not password[rule[0] - 1] == rule[2] and password[rule[1] - 1] == rule[2]):
        return True
    return False
